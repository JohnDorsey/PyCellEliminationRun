...
def enumerateDeeply(data, uniformDepth=True):
    if uniformDepth:
      assert all(isinstance(subItem, list) for subItem in data) or all(not isinstance(subItem, list) for subItem in data) #@ replace with pyGenTools.allAreEqual?
    if isinstance(data, list):
      ...
      raise ValueError("enumerateDeeply arg incompatible ({}).".format(repr(type(data))))
...
def setValueUsingPath(data, path, value, requireSameType=True, allowOverwriteNone=True): # returns original value
  ...
  if not isinstance(workingData, list):
  ...
  if isinstance(workingData[path[-1]], list):
  ...
  if requireSameType:
    assert type(workingData[path[-1]]) == type(value) or (allowOverwriteNone and workingData[path[-1]] is None)
